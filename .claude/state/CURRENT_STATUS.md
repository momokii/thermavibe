...
**Updated By:** Share branding moved from env vars to OperatorConfig (D-030)
**Session Summary:** Superseded D-028 — share landing page branding (`share_brand_name`, `share_brand_handle`, `share_brand_color`) now lives in `operator_configs` under a new `SHARING` category, editable at `/admin/sharing`. The env vars were removed from `config.py`, `.env.example`, `.env.production`. `share_page.py` signature changed from `Settings` to `dict[str, str]`; `share_landing_page` endpoint now fetches branding from `config_service.get_configs_by_category(db, 'sharing')` before rendering. New `SharingConfig.tsx` component + `AdminSharingPage.tsx` page, wired into the admin sidebar (`Share2` icon) and route table. Pattern cloned from `PrintTemplateConfig.tsx`. Backend tests: 323 passing (+5 from this batch: 1 new `test_empty_branding_dict_uses_all_defaults`, 1 new `test_landing_page_uses_db_sourced_branding`, updated `test_has_eight_categories`/`test_has_all_expected_categories` for new SHARING category, fixed pre-existing test-isolation bug in `test_share_url_is_relative_by_default` that surfaced when `PUBLIC_BASE_URL` is set in `.env`). Frontend tests: 36/36 unchanged. RevealScreen lint baseline unchanged. Existing operators who set `SHARE_BRAND_*` in `.env` need to re-enter values once via `/admin/sharing`.
